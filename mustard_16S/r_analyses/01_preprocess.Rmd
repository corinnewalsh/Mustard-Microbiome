...
cat('Summary read depth statistics: host reads removed, pooled')
...
## contaminant dataframe of main contaminants identified in blanks
contam_df <- blank_df %>% 
  filter(asv_ubiq > 25)
...
##### note: also created relative abundance dataframes
...
### relative abundance data - sample specific data frames
```{r}
### all data
...
### f10 data
input_f10rl_samps1 = filter_data(input_f10_relab, 'sample_type_2', keep_vals = c("rhizo","root","seed","slurry", "soil"))
input_f10rl_samps2 = filter_data(input_f10_relab, 'sample_type_2', keep_vals = c("rhizo","root","slurry","soil"))
input_f10rl_samps3 = filter_data(input_f10_relab, 'sample_type_2', keep_vals = c("rhizo","root"))

input_f10rl_samps4 = filter_data(input_f10_relab, 'sample_type_2', keep_vals = c("slurry","soil"))
input_f10rl_samps5 = filter_data(input_f10_relab, 'sample_type_2', keep_vals = c("rhizo"))
input_f10rl_samps6 = filter_data(input_f10_relab, 'sample_type_2', keep_vals = c("root"))
input_f10rl_samps7 = filter_data(input_f10_relab, 'sample_type_2', keep_vals = c("seed"))
```
...
### rarefied data - sample specific data frames 
```{r}
### all rarefied data
...
## distance matrix for plotting
# sb_transformed.s7 <- t(sqrt(input_f2rl_samps7.1$data_loaded))
# dm.s7 <- vegdist(sb_transformed.s7, method = "bray", na.rm = TRUE)
# sb.s7.nmds <- metaMDS(dm.s7, k = 3, trymax = 50)
# 
# 
# ## plot nmds
# nmds_s7 <- plot_ordination(input = input_f2rl_samps7.1, ordination_axes = sb.s7.nmds$points[,1:2], 
#                            color_cat = "condition", shape = "sterility", hulls = TRUE)+
#   scale_colour_manual(values=pair.pal2)+
#   scale_fill_manual(values=pair.pal2)+
#   ggtitle("NMDS plot of seed samples")
#   #theme(legend.position = "none")
# nmds_s7
# 
# ggsave(nmds_s7, filename = paste0(figures_01_clean.fp, "/nmds_seed_f5.png"), height = 8, width = 10)
```