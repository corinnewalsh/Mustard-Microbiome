...
```{r}
#### Visualization: NMDS seed!
## distance matrix for plotting
seed_transformed <- t(sqrt(input_seed$data_loaded))
dm.seed <- vegdist(seed_transformed, method = "bray", na.rm = TRUE)
nmds.seed <- metaMDS(dm.seed, k = 3, trymax = 500)

## plot nmds
nmds_seed <- plot_ordination(input = input_seed, ordination_axes = nmds.seed$points[,1:2], 
                             color_cat = 'condition', 
                             shape_cat = 'sterility')+
  scale_colour_manual(values=pair.pal2)+
  ggtitle("NMDS plot of seed samples (rarefied 1k)")
nmds_seed

ggsave(nmds_seed, filename = paste0(figures_02.fp, "/nmds_seed.png"), height = 8, width = 10)
```
